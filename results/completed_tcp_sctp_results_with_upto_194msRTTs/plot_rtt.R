...
title<-paste("CNT:",args[1],"PLR:",args[2],"CSZ:",args[3],sep=" ")
dat<-data.frame(cond=rep(c("Stackoverflow","go","guardian"),each=4),xvar=x$x1,yvar=x$x2)
pdf(args[4],width=6,height=4,paper='special')
ggplot(dat, aes(x=xvar, y=yvar/1000, color=cond))  +geom_point(size=2)+geom_line(aes())+ 
	theme_bw()+labs(x="RTT [ms]", y="PLT [s]",linetype='custom title',fill="")+ ggtitle(title) + 

...
run_worker(void *arg)
{
	...
	if(j!=-1){
		...
		if(protocol==HTTP1){
			snprintf(url, sizeof url,"%s%s","http://193.10.227.23",
					cJSON_GetObjectItem(this_obj,"path")->valuestring);
		} else if(protocol==HTTPS){
			snprintf(url, sizeof url,"%s%s","https://193.10.227.23",
					cJSON_GetObjectItem(this_obj,"path")->valuestring);
		}
...
checkDependedActivities(char *id)
{
	...
	if(i!=-1){
		objs= cJSON_GetArrayItem(this_acts_array, i);
		obj= cJSON_GetObjectItem(objs, id);
		deps=cJSON_GetObjectItem(obj,"deps");
...
createActivity(char *job_id)
{
	...
	i=cJSON_HasArrayItem(this_acts_array,job_id);
	if(i!=-1){
		cJSON *obj;
		cJSON *obj_name;
		cJSON *this_obj;
			...
			if(i!=-1){
				...
				while(1){
					if (global_array_sum()<MAX_CON){ 
						error = pthread_create(&tid2,NULL,run_worker,(void *)obj_name); // Felix: here the thread (run_worker) is called when a http request is met
						if(0 != error){
